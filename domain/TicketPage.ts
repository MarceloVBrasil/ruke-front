...
export function filterByNameClient(tickets: any[], nameClient: string) {
  const newFilteredTickets = nameClient ? tickets.filter(
    (ticket: any) =>
      (ticket?.name_client?.toLowerCase().includes(nameClient.toLowerCase()))
  ) : tickets;
  return newFilteredTickets;
}

export function filterByTagClient(tickets: any[], tagClient: string) {
  const newFilteredTickets = tickets.filter(
    (ticket: any) => ticket.tag && ticket.tag.indexOf(tagClient) > -1
  );
  return newFilteredTickets;
}
