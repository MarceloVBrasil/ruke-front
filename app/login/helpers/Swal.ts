...
export const handleSingIn = async (props: {
    email: string,
    senha: string,
    ...
    if (response.error) {
        ...
        if (response.cadastroPendente) return goToTenantsUniquePage(response.tenantId) //router.push(`/tenants/${response.tenantId}`)
        else return goToHomePage() //router.push('/')
    }
}
