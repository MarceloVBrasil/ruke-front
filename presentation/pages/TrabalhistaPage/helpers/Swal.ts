...
import { addTrabalhistaTicket } from "@/app/api/server/trabalhista";
...
export const handleDelete = async (id: string, onDelete: (id: string) => Promise<void>) => {
    try {
        await Swal.fire({
...
export const handleAddTicket = async (setLoading: Dispatch<boolean>, cb: (id: string) => void) => {
    try {
        setLoading(true);
        const response = await addTrabalhistaTicket();
        if (response.id) {
            cb(response.id)
        }
