...
import { isCepValid, isCnpjValid, isFieldEmpty, isNegative, isPositive } from '@/app/utils/validators';
import { formReducer, getErrorInitaialState, getFormInitialState } from './pjReducerFunctions';
...
interface IModal {
    open: boolean
    onClose: () => void
    onAdicionarClick: (PJ: PJ_RECLAMADA) => void
    existem_outras_adicionadas: boolean
}

export function AddPJModal(props: IModal) {
    const { open, onClose, onAdicionarClick, existem_outras_adicionadas } = props
    const [state, dispatch] = useReducer((state: PJ_RECLAMADA, action: Action) => formReducer(state, action, existem_outras_adicionadas), getFormInitialState(existem_outras_adicionadas));
    ...
    return (
        <AddModal
            title='Adicionar reclamada pessoa jurídica'
            open={open}
            onClose={onClose}
    ...
    function handleAdicionarClick() {
        ...
        onAdicionarClick(state as PJ_RECLAMADA)
        onClose()
        resetForm()
    }
