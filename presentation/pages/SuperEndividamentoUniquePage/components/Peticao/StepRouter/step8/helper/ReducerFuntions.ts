import { Action, FormState, FormField, ErrorStep8 } from "./FormTypesAndFields";

function getFormInitialState(): FormState {
    const initialState: FormState = {
        [FormField.ACOES_JUDICIAIS_CLIENTE]: {
            value: [],
            changed: false
        }
    };
...
function getStateFromApi(api_data: any) {
    const data: FormState = {
        [FormField.ACOES_JUDICIAIS_CLIENTE]: {
            value: api_data[FormField.ACOES_JUDICIAIS_CLIENTE] || [],
            changed: false
        }
    };
...
function getErrorInitialState(): ErrorStep8 {
    const erros: ErrorStep8 = {
        acoes_judiciais_cliente: false
    }

    return erros
}

export { formReducer, getFormInitialState, getStateFromApi, getErrorInitialState }