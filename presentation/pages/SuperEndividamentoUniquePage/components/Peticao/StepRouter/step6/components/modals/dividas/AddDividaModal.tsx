...
import { formReducer, getErrorInitialState, getFormInitialState } from './DividaReducerFunctions';
import { isOutraNatureza } from '../../../helper/ReducerFuntions';
import { AddModal } from '@/presentation/components/ModalAdd';
import { isFieldEmpty, isNegative, isPositive } from '@/app/utils/validators';
...
interface IModal {
    open: boolean
    onClose: () => void
    onAdicionarClick: (divida: divida) => void
    credores: string[]
}

export function AddDividaModal(props: IModal) {
    const { open, onClose, onAdicionarClick, credores } = props
    ...
    const [naturezaCompraBemChecked, setNaturezaCompraBemChecked] = React.useState(false)
    const [naturezaOutrosChecked, setNaturezaOutrosChecked] = React.useState(false)
    ...
    function handleOutraNaturezaTypeChange(e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, prefix?: string) {
    ...
    function handleCompraDeBemNaturezaTypeChange(e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, prefix?: string) {
        const { value } = e.target
