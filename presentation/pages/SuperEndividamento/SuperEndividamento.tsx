...
import { deleteSuperendividamentoTicket } from "@/app/api/server/superendividamento";
...
type SuperEndividamentoProps = {
    listTickets: SuperEndividamento[];
    regraDominio: any;
};
...
export default function SuperEndividamentoPage({
    ...
    const [tickets] = useState<SuperEndividamento[] | []>(listTickets);
    ...
    const onAddTicket = (ticket_id: string) => {
        router.push(`/superendividamento/${ticket_id}?step=1`);
    }

    useEffect(() => {
        setFilteredTickets(ticketsView.filter((ticket: any) => ticket.nome_cliente?.toLowerCase().includes(nameClient.toLowerCase())))
    ...
    async function onTicketDelete(id: string): Promise<void> {
        await deleteSuperendividamentoTicket(id)
