...
export default function SmallScreen(props: ISmallScreen) {
    ...
    return (
        <Grid item xs={12} key={ticket.id}>
            <Paper sx={{ padding: "10px", marginY: "10px", display: 'flex', flexDirection: 'column', gap: 4 }}>
                ...
                >
                    {regraDominio?.permissoes?.includes("getById") && (
                        <Link
                            ...
                            href={`superendividamento/${ticket.id}?step=${getPassoFromEtapa(ticket.etapa)}`}>
                            <Button
                                variant="contained"
                                color="primary"
                    ...
                    {regraDominio?.permissoes?.includes("delete") && (
                        <Button
                            ...
                            onClick={() => handleDelete(onTicketDelete, ticket.id)}
                            sx={{ width: '100%' }}
                        >
                            Excluir
                        </Button>
